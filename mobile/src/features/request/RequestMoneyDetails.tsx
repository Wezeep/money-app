...
const handleSendRequest = async () => {
    ...
    try {
      ...
      for (const contact of contacts) {
        ...
        if (!contact.isWezeepUser || !contact.wezeepId) {
          skipped += 1;
          continue;
        }
        const res = await moneyRequestsApi.create({
          recipientWezeepId: contact.wezeepId,
      ...
      if (responses.length === 0) {
        setSendError("No Wezeep contacts selected. Please choose a Wezeep user.");
        return;
      }
      if (skipped > 0) {
        alert(`${skipped} contact(s) were skipped because they are not Wezeep users.`);
      }
